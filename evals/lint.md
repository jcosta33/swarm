...
None of the four cross-step predicates are scored at the `lint` output: `lint` emits no SOL artifact (parse-validity is scored on the artifact-emitting steps), reaches no verdict, and detects no drift. Its rubric is fully covered by L1–L4 above.
