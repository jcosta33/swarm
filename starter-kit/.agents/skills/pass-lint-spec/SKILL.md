...
description: >-
  ...
  named defects (`improve`), building the structured form (`lower`), partitioning into
...
How to run the `lint` step well. It is **SOFT control**: procedure, not meaning. It MUST NOT define modality, authority order, verification semantics, lint-code meaning, or any other load-bearing fact — those are fixed by SOL and the structured form; this guide only *applies* them. Every code, severity, gate, and record shape below has a fixed meaning this guide applies and never redefines:
...
- The `lint` step contract — non-mutating, `PARSE`+`NORMALIZE`, Skeptic carrier, no runtime — is fixed by the `lint` step.
- The CLARIFY gate is fixed by the `lint` step (and the blocking-question rule, R-BLOCKING-Q, also lives there).
...
- No edit to the spec. `lint` is **non-mutating** (see the `lint` step): change a character and you left the `lint` step.
...
A tripped gate surfaces as the *existing* code that tripped it (`SOL-O003`/`SOL-M002`/`SOL-P008`), already in your report from steps 2–3 — the gate aggregates these three into one checkpoint; it is **not a fourth code**. Do not conflate it with the `CLARIFY` *improve op* (the `improve` step): the op *creates* the explicit interpretation or `QUESTION` from one buried `SOL-P008`; the gate *waits on* such questions being discharged (the `lint` step). Verify the predicate by hand against the spec/structured form; state it as a review-checkable contract and never claim a tool enforces it. *Why:* the planner→coder handoff is the dominant failure surface — structuring past an unresolved ambiguity commits a guess as an obligation, the single most expensive defect to discover downstream.

### Project commands

`lint` needs **no `cmd*` slot** in the common case: it is a non-mutating, review-checkable contract performed by hand today (Invariant 1 — no runtime, parser, or checker is shipped; see the `lint` step). If the consuming repo has wired a static-analysis slot (`cmdLint`/`cmdValidate`), you MAY paste its real output as corroboration — but its absence never blocks the step, and you MUST NOT present any tool as *enforcing* the lint result. If a task asks you to run a project lint command and the relevant slot is undefined in the consuming repo's `AGENTS.md > Commands`, **ask the user** — never guess a command.

## Output contract

- A lint report: an array of `{ code, severity, layer, span, message, suggest }` records (each valid against the shape fixed by the SOL error catalogue) plus the overall blocking status.
- Codes are unified `SOL-<LAYER><NNN>` only; no legacy aliases, no invented codes, no structured form `note` level.
...
- ❌ Editing the spec to fix a defect you found ("it was a one-word fix"). → Emit the diagnostic with a `suggest`; the fix is the `improve` step (the `improve` step). One changed character means you left the `lint` step (the `lint` step).
...
- ❌ Emitting a structured form `note`-level diagnostic. → `note` has **no surface producer in v0.1** (the SOL error catalogue); a valid checker MUST NOT emit it.
- ❌ Adding a fourth "CLARIFY" code for a tripped gate. → The gate aggregates `SOL-O003`/`SOL-M002`/`SOL-P008`; it introduces no new diagnostic (the `lint` step).
- ❌ Claiming the lint or CLARIFY gate is "enforced by tooling" or "the linter passed." → There is no runtime (Invariant 1); both are review-checkable contracts performed by hand today (see the `lint` step). State them as contracts, not tool output.
