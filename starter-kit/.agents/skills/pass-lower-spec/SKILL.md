...
How to run the `lower` step. **SOFT control** — procedure, not meaning. The structured-form shape, the 7 edge types,
and the structuring contract are fixed by `reference/ir.md`; this guide applies them.

## Purpose
`lower` derives the **typed obligations** — the machine-checkable form of the same content the spec
states in prose. Every later step reasons over the structured form (`decompose` plans from it, `verify`/`review` judge
...
5. **Preserve binding content (the loss budget).** Every obligation, modality, and `VERIFY BY` binding in
   the source MUST be recoverable from the structured form. Permitted drops are only non-binding (rationale, source
...
## Anti-patterns
- ❌ Structuring an obligation an open `[blocking]` QUESTION affects → `SOL-O003`; resolve it first.
...
- ❌ Dropping a binding to "simplify" the graph → structuring is lossless for binding content.
...
- Every source obligation/modality/binding recoverable from the structured form?
- No in-scope obligation structured while a blocking QUESTION affects it?

## Related
- structured-form shape, edges, structuring contract — `reference/ir.md`. Surface forms — `reference/sol.md`.
