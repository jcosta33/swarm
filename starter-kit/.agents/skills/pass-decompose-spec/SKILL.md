...
5. **Apply the single safe-parallelism predicate to set `merge_safe`.** Use Swarm's **one** predicate verbatim — no relaxed alternative is permitted (see the `decompose` pass). Two packets `a` and `b` may run in parallel **iff** they are dependency-independent **and** write-disjoint:
