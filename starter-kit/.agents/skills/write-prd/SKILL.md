...
# Step guide: write-prd (`author` · intent parent of a spec)
...
- The `author`-step and `prd.md` artifact contracts — for lookup, never redefined here.
...
Run these steps in order. Each rule carries its WHY so you can extend it to a case the contract did not anticipate. Throughout, hold the one boundary that defines the artifact: a sentence describing a *mechanism* or reading as an *obligation* belongs not in the PRD but in the spec the `author` step will produce.
...
Before closing the step, confirm — and where a step would otherwise leave no visible trace, write the trace into the deliverable:
