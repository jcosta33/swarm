...
How to execute the `review` pass and decide its merge gate. It is a **pass guide** (§26.5): SOFT control conditioning how an agent runs the pass; it constrains nothing on its own (§17.1). It does **not** redefine a verdict's meaning, the proof-strength order, the gate predicate, or waiver/contradiction semantics — those are fixed by the verification rules (§14/§15/§17), which this guide only applies. Every "gate", "check", or "enforcement" below is **manual today**: a contract performed by hand, with a deterministic home (CI, a PreToolUse hook, a merge-blocking status) when a harness exists. Never claim it runs today (§17.1).

This pass runs under the **Skeptic stance** (`review[profile: skeptic]`, §9.4). The adversarial method is the Skeptic stance applied to `review` — there is no separate adversarial-review pass (§26.2). The discipline that stance demands is restated inline below, and is sufficient to run the pass: mistrust the diff and the worker's framing, re-run every bound validation yourself rather than trusting a paste, anchor every finding to `file:line`, and paste the real run output as evidence.
...
- `*.swarm.trace.md` — the implementation's claims (the *input* to judgment; a `TRACE` is never itself a verdict, §14).
- The `spec.swarm.md` obligations in scope — every `REQ`, `CONSTRAINT`, `INVARIANT`, `INTERFACE` with its required `VERIFY BY` bindings (§14.4).
- The recorded verification evidence (§15) and the diffs.
- The `review.md` artifact contract (§21.5) for the output shape.
- The default proof suite for the `review` task kind: `manual @ REVIEW` over the recorded evidence, plus re-run of the bound `cmd*` proofs (§15.8).

The verdict vocabulary, proof-strength order, gate predicate, and waiver/contradiction semantics are fixed by the verification rules (§14/§15/§17); this guide applies them, never restating their meaning.
...
## Rejects — non-proofs that never earn a `PASS` (§15.9)
...
A reviewer who cannot tell whether a binding `BLOCKED` (environment fix) or is `UNVERIFIED` (binding/execution gap) MUST record `UNVERIFIED` — the weaker, more honest claim (§14.1.1). *Why:* `BLOCKED` quietly claims "the truth is merely unknown"; defaulting to it lets an unbound proof masquerade as a transient environment problem.

## Procedure

1. **Read the trace and the obligations side by side.** For every required obligation in scope, find the trace claim that purports to satisfy it and the binding(s) it cites. List the required `VERIFY BY` bindings: **one verdict per binding** (§15.7), so an obligation with three required bindings owes three verdicts. *Why:* counting verdicts against bindings surfaces an obligation claimed but only partly proven.
...
- ❌ **"Tests passed" as a verdict** → `UNVERIFIED` until a command, exit code, and run output appear in `EVIDENCE`. The bare phrase is the headline non-proof (§15.9).
- ❌ **Guessing a project command** when `AGENTS.md > Commands` has no slot for it → ask the user; never substitute a command you assume.
- ❌ **Picking the convenient side of two disagreeing proofs** → exactly what `CONTRADICTED` forbids; record both refs and route per §17.4. The stronger oracle is a *working assumption*, not a resolution.
- ❌ **Inventing or re-defining a verdict value** (a fifth core, a new decorator, "PASS-ish") → the seven values and their decoration rules are fixed in §14.1; this guide only applies them.
...
- ❌ **Creating a `verdict.md`** → `review.md` *is* the verdict record (§14.5); a standalone `verdict.md` is non-conformant.
- ❌ **Claiming the gate "runs" or "is enforced" today** → it is manual today (§17.1); name its future deterministic home, do not assert automatic enforcement.
...
- Is there **one verdict per required binding** (§15.7), and does `## Final verdict` follow strictly from the gate predicate (§14.4) — no `FAIL`/`BLOCKED`/`UNVERIFIED`/`STALE`/`CONTRADICTED` left in a required binding while you wrote `PASS`?
