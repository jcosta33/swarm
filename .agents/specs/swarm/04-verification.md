...
> **Merge gate (normative).** A change set MAY be promoted **if and only if**, for **every required `VERIFY BY` binding** of every required obligation, the binding's latest verdict is `PASS` or `WAIVED`, **and none** is `STALE`, `CONTRADICTED`, `FAIL`, `BLOCKED`, or `UNVERIFIED` (§15.7 — an obligation with three required bindings contributes three verdicts). The node-level `status` is the **aggregate** over the obligation's bindings: `FAIL`/`BLOCKED`/`UNVERIFIED`/`STALE`/`CONTRADICTED` if any binding is, else `PASS`. "Latest" is the verdict from the most recent recorded run for that binding (ordered by the recorded verdict timestamp).
...
> Rationale: an `INVARIANT` asserts a universally-quantified property ("for all states, P holds"); a single example-based unit `test` cannot establish a universal, so `property`/`model`/`static` are preferred and a test-only binding is flagged but not blocked (the INVARIANT-proof-preference rule, §15.4). An `INTERFACE` is a boundary contract, so its proof MUST be a `contract` check that exercises `RETURNS`/`ACCEPTS`/`ERRORS` (closing the INTERFACE-verification gap, `SOL-V006`).
...
Every `VERIFY BY` binding's **last `PASS`** MUST record enough provenance to detect later drift. This is **the one trace-provenance schema** that §14, §16, and §23 all reference — there is exactly one, so the verdict model, the drift check, and the memory model never diverge. The closed seven-field set below (G11) is the **required base**; two named **optional adjuncts** MAY extend a verdict's provenance without altering that base — `oracle_adequacy` (§15.10.1, present when adequacy is recorded) and `judge` (§17.6.1, present when the oracle is a `manual`/judge-rendered verdict). The adjuncts are optional extensions on the same record, not additional schemas, so "exactly one" still holds.
...
- **A SOFT-control artifact MUST NOT define hard semantics.** No skill, persona/profile, or `AGENTS.md` section may define modality, authority order, or verification semantics — those live in SOL and the typed IR (§2, §7.1). A regression check MUST confirm this (§34).

> Rationale: Anthropic's own guidance frames memory/instructions as context, not enforced configuration, and points to a PreToolUse hook for anything that must hold `[ANTHROPIC-MEM]`. Multi-turn reliability decay and prompt-format sensitivity make a model an unsound enforcement substrate. Honesty about this boundary is what lets Swarm be trusted.
...
3. **The stronger oracle is authoritative pending reconciliation.** Using the proof-strength order (§15.6, `model > property|contract > test > static > manual|monitor`), the **stronger** proof's result is treated as authoritative *while the contradiction is open*. This does not close the contradiction; it sets the working assumption so review is not paralysed. Example: if a `contract` proof says `PASS` and a `manual` judgment says `FAIL`, the `contract` result is presumptively authoritative, but the obligation stays `CONTRADICTED` (and gate-blocking) until reconciled. **Equal-strength case:** when the two proofs share a rank (e.g. `property` vs `contract`, `manual` vs `monitor`, or two `test`s), neither is stronger — no working assumption is set; the contradiction stays open and routes to an independent reviewer or a higher-rank re-proof, and MUST NOT auto-resolve to either side.
...
For any change whose domain is `security` (Axis B rank 3, §22) or that touches an attack surface, the stdlib SHOULD make available a conditional **Tier-3 `threat-model.md`** source-doc, mapped to `[OWASP-LLM01]`. Like the other Tier-3 source-docs (§20.3.3, §21.9), it is plain `.md`, carries `type` + `id` frontmatter, holds **no obligation blocks**, preserves a fixed epistemic stance (*threat observation*, not intent), and is **CONDITIONAL** — never conformance-required: it sits **outside** the five-template Tier-3 inventory the conformance definition counts (§20.3.4, §20.4), so a conformant kernel payload MAY ship it as an optional security extension, and a conformant repo MAY have zero instances. It promotes forward only through an `author` pass that re-states each modelled threat as a `CONSTRAINT`/`INVARIANT` with its own id, modality, and `VERIFY BY` (typically a `security` proof, §15.1) — and, being an externally-informed observation, is subject to the §17.5.2 source-authority rule before any obligation it implies becomes binding.
