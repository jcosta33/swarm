...
For the core `cmd*` slots and the resolver contract see §31.3 and §15.3; the extended and out-of-contract tiers are defined inline above (this §32.4).
...
| **2 — Swarm-lintable** | Authored specs are structurally and prose-valid: the obligation language parses and carries no blocking surface/prose defect. | Every approved `*.swarm.md` emits **zero blocking `SOL-S*` and zero blocking `SOL-P*`** diagnostics (§8; the `SOL-<LAYER>NNN` scheme of §32.6), where *blocking* is the `severity` field's `BLOCKING` value (lowering to the IR `level` of `error`, Appendix B.1). `SOL-M*`/`SOL-V*`/`SOL-O*` are not gated at this tier. (`APS` is the prose-standard NAME, not a code prefix — its violations surface as `SOL-P*`.) |
...
**Provenance-id grammar (normative).** A `research.md` tags citable evidence spans with ids `R-001`, `R-002`, … (the `R-` prefix marks an evidence span — parallel to the §4.1 block-id prefixes, but for a non-SOL evidence source). A derived obligation cites a span with the cross-file reference `research#R-NNN` — a new provenance form that reuses the `<stem>#<local-id>` *shape* of the §5.7 cross-spec reference but is NOT a §5.7 production: §5.7 admits only `<spec-id>#<obligation-id>`, where the left operand is a declared spec id and the right is a §4.1-prefixed block-header id, whereas here `research` is an evidence-source stem (not a spec id) and `R-` is a provenance prefix (not a §4.1 SOL block prefix) — in its `BECAUSE` clause. A citation whose `<span-id>` is absent from the named `research.md`, or whose stem names no shipped source, is a provenance failure.
