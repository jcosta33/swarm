# 🛠️ Skill: write-rewrite

> **Auto-loaded for `rewrite` tasks.** Codifies the discipline that distinguishes rewrites from refactors: the behaviour delta is *explicit* before code changes; everything not in the delta must be preserved.
...
A spec at `.agents/specs/cache-rewrite.md` rewrites the project's caching layer to support TTL-per-key (currently global TTL only).

Behaviour delta:

| Aspect | Before | After |
|--------|--------|-------|
| TTL semantics | global, set at cache init | per-key, defaults to global if unspecified |
| `get(key)` return | `T \| undefined` (undefined on miss or expired) | unchanged |
| `set(key, value)` signature | `(key: string, value: T)` | `(key: string, value: T, ttlMs?: number)` |
| `set` error on bad value | undefined behaviour | throws `CacheError(code: 'invalid_value')` |

The Builder:

1. Identifies all callers of `set` (15 sites). Most pass 2 args; the spec authorises this (the new arg is optional). 2 callers pass invalid values relying on undefined behaviour — these get explicit handling per the new throw contract.
2. Implements the rewrite in batches; runs `{{cmdValidate}}` and `{{cmdTest}}` after each.
3. The original tests still pass (preserved behaviours). New tests cover the per-key TTL and the throw-on-bad-value behaviours.
4. Mid-implementation, discovers that `cache.clear()` semantics are subtly different in the new implementation (it now clears a different scope). This is *not* in the delta. The Builder *halts*, updates the spec to either authorise or forbid the change, and resumes only after.

This is the rewrite discipline: every behaviour change is intentional and authorised; everything else is preserved.

---

## See also

- [`tasks/rewrite.md`](../tasks/rewrite.md)
- [`personas/the-builder.md`](../personas/the-builder.md)
- [`tasks/refactor.md`](../tasks/refactor.md) — sibling for behaviour-preserved restructuring
