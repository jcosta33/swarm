# 🛠️ Skill: adversarial-review

> **Auto-loaded for review, audit-writing, deepen-audit, bug-report-writing, and fix tasks.** The Skeptic's discipline. Walk a diff (or an audit, or a codebase area) with the six adversarial questions. Findings cite file and line. Run validation yourself; do not trust upstream.
...
- **Severity** (BLOCKER / MAJOR / MINOR — see [tasks/review.md](../tasks/review.md))
- **File:line**
...
See [The Skeptic's worked example](../personas/the-skeptic.md#%EF%B8%8F-example-how-the-skeptic-resolves-a-representative-issue) — the rate-limiter review that caught a Cloudflare-IP bug despite the worker's "all tests pass" claim.

The example illustrates each of the six adversarial questions:

1. **Intent:** "Add a sliding-window rate limiter to /api/login, keyed by IP."
2. **Does the code do it?** Mostly yes, but the IP extraction is wrong (Finding #1).
3. **What didn't change that should have?** The codebase pattern of `req.realIp ?? req.ip` — used in 14 other places — was diverged from. Cross-module search confirmed.
4. **What edge cases are unhandled?** Behind Cloudflare, every request shares the same proxy IP — entire bucket is global. Unhandled.
5. **What production failure modes are possible?** Per-request `setImmediate` cleanup is O(n²) under load (Finding #2).
6. **What was claimed but not verified?** Worker's "manually tested with 6 rapid requests" — the manual test didn't exercise the Cloudflare path; the *passing tests* didn't cover the failure mode either (Finding #3).

Each question yielded specific findings; severity calibrated; verdict rendered with reasoning.
...
- [`personas/the-skeptic.md`](../personas/the-skeptic.md) — the canonical user
- [`personas/the-auditor.md`](../personas/the-auditor.md) — uses the adversarial mindset for audits
- [`personas/the-bug-hunter.md`](../personas/the-bug-hunter.md) — uses the adversarial mindset for root-causing
- [`tasks/review.md`](../tasks/review.md) — where this skill is the workhorse
- [`empirical-proof.md`](empirical-proof.md) — sister skill
- [`concepts/09-empirical-proof.md`](../concepts/09-empirical-proof.md) — the framing
