# Source artifacts and the two-class partition

> Swarm's reference for source artifacts: which files a Swarm repository may contain, how the framework tells them apart (frontmatter `type`), the canonical filenames per class, and the tiers on which conformance is defined.
...
| --- | --- | --- |
| **Spec** | Frontmatter `type: spec` — the sole source of SOL obligation blocks. | The file is *parsed or emitted by the build steps*. Its bytes are subject to the SOL grammar (see the [SOL reference](./language/SOL.md)) or the structured-form / plan JSON schemas (the [structured form](./reference/structured-form.md)). |
| **Working artifact** | Frontmatter `type` is anything else (`task`, `review`, `finding`, `adr`, `audit`, `research`, …) or absent. | A *human/agent working artifact*: structured Markdown governed by an artifact contract, not by the SOL grammar — though it MAY embed SOL blocks (notably `VERDICT` and `TRACE`) as quoted data. |

A conformant Swarm tool MUST use frontmatter `type: spec` as the **sole, sufficient discriminator** for "do I parse/emit this": it MUST NOT parse a plain `.md` working artifact as SOL source, and MUST NOT emit a build output to a path whose frontmatter lacks `type: spec`.

The only **human-authored** spec artifact is `spec.md` (the source spec); the `*.ir.json` / `*.trace.md` variants are *emitted* artifacts (see §3.1 below).
...
- **7 block types** — the SOL surface-language blocks (`REQ`, `CONSTRAINT`, `INVARIANT`, `INTERFACE`, `QUESTION`, `TRACE`, `VERDICT`) are what a `spec.md` source carries and what working artifacts embed as quoted data.
...
| --- | --- | --- | --- | --- |
| `spec.md` | Source spec — APS prose interleaved with SOL blocks. | Human / authoring agent | SOL grammar + APS prose standard | Live; the only hand-written spec file. |
| `*.ir.json` | Emitted structured form (the spec's obligations). | Build step (future tool) | structured-form envelope | **Reserved contract name.** Not written by any shipped tool. |
| `*.plan.json` | Emitted plan (structured, schedulable work packets + graphs). | Build step (future tool) | plan envelope | **Reserved contract name.** Not written by any shipped tool. |
...
| # | Artifact | Class | Role |
| --- | --- | --- | --- |
| 1 | `spec.md` | Spec | Source of obligations. |
...
| --- | --- | --- | --- |
| 1 | `audit.md` | Observation-only (records what *is*, never prescribes). | a `spec.md` (via `author` step). |
| 2 | `research.md` | Investigation (open questions + findings). | a `spec.md` (via `author` step). |
...
A spec is not born only from research. Swarm normalizes many requirements-practice artefacts into one obligation-bearing `spec.md` rather than pretending every intent begins as research. The distinction this projection carries forward is:
