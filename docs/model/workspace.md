...
| `status/` | **Observed** satisfaction + drift: per-spec satisfaction reports, task/worktree state, drift reports. Records whether code satisfies the spec; never redefines intent. A `worktrees/` entry records its `branch`, `base`, owning task id, and liveness, so an in-flight or long-lived (multi-session, e.g. a migration) worktree resumes from committed state rather than the gitignored task file. | observed | Yes | The `verify`/`review` passes; drift detection (future toolchain) |
...
Because every durable artifact lives in `.swarm/` rather than in a model's context window, a task that spans more than one session does not have to be re-derived when a fresh session picks it up. The workspace *is* the resumption record. Context is a finite resource, so the discipline is to externalise intermediate work into files and load only what the task needs back into the window [[CTXENG]](../research/sources.md#CTXENG); writing the intermediate steps down rather than holding them implicitly is itself what makes the multi-step work tractable [[SCRATCHPAD]](../research/sources.md#SCRATCHPAD). A new session reconstructs where the work stands by reading the same three durable surfaces that govern the work in the first place: the `sources/` artifact that fixes desired truth, the `status/` entry that records observed satisfaction and the task's current state, and — once a unit completes — the `ledger/` entry that compacts what the reconciliation produced. Decisions, findings, and the next concrete starting points are written into that durable state as the work proceeds, so the file content is full even when the context window is empty.

This is the same file-state-externalization discipline as the rest of the workspace, applied to continuity in time: the kernel deliberately does not try to solve long-context coherence at the model layer. Sessions time out and workers swap mid-task; the response is to externalize state so the next session lands in the same epistemic position the last one left, loading what the task names rather than re-investigating it. Persisting task state to disk with explicit inter-task dependencies — the `task.md` frame and the task DAG — mirrors a pattern validated at vendor scale [[CCTASKS]](../research/sources.md#CCTASKS). For how a task's own file carries this resumption state across sessions, see [the evidence](../research/task-files.md).
