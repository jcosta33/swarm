...
| C002 | `duplicate-id`           | No other file in the workspace claims the same frontmatter `id:`, and no requirement ID is reused across **non-draft** specs — a `draft`'s stub ids (a fresh scaffold's `AC-001`) are exempt as not-yet-finalized claims; the rule applies to every spec once it leaves `draft` (ready, done, …). | hard error       |
