...
| `## Observations` | What is true *today*, each item citing the evidence that grounds it (`file:line`, command output, grep result, or other observable). | Present state only. State the fact, never the fix. Each observation MUST be evidence-cited — an ungrounded, fact-shaped claim is a defect. |
