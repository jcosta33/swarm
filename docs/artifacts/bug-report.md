...
| `## Affected obligations` | The existing obligation the defect violates, listed as a reference only — the spec id plus the local obligation id (`<spec-id>#<AC\|C\|I\|IF>-NNN` — the id prefix, not the block-type name) — and how it is violated. **This list is what populates the promoted fix task's `assigned_obligations`.** If no obligation covers the broken behavior, say so explicitly: that gap is a finding the fix task must reconcile by authoring the covering obligation first (a spec amendment), since an uncovered change cannot pass the merge gate by vacuity. **Author no obligation blocks here.** |
