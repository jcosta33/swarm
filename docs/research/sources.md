...
**[PERSUASIONPARADOX] The Persuasion Paradox: When LLM Explanations Fail to Improve Human-AI Team Performance.** **arXiv:2604.03237** (preprint, 2026). *Verified (June 2026, id + abstract).* Reproduces the older explanation/over-reliance finding with **modern LLMs**: a fluent-but-incorrect rationale masks the error and inflates trust, so richer narrative explanations do not improve team performance and can hurt; simple uncertainty/confidence cues sometimes calibrate reliance better than long justifications. Grounds (preliminary): the over-reliance findings replicate and intensify as models get more capable — higher fluency makes errors harder to spot — so the design lesson (short calibration cues over persuasive prose) carries into the LLM-agent era. Never a `MUST`.
