...
Before doing the work, decide *where* it happens. Isolation is a three-rung axis — `worktree+branch`, `branch-only`, `in-place` — **orthogonal to `parallel_group`** (a single non-parallel task can still need a worktree). An agent resolves it deterministically by reading the task frame and the request, **first match wins** (the full model is [ADR-0046](../adrs/0046-isolation-axis-model.md)):
