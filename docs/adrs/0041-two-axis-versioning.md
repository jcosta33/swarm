...
- **Language version** — the SOL + APS feature set. Carried **per file** in frontmatter as `suspec_language` (the SOL discriminator, e.g. `SOL/0.1`) and `aps_version` (e.g. `0.1`), so one repo MAY hold `spec.md` files at different language versions mid-migration (§25.1.1).
- **Framework / package version** — the kernel payload, templates, pass guides, profiles, flow-graph. A single semver in `starter-kit/.agents/.suspec-version`, mirrored by an adopted project as `.agents/suspec.version` (§25.1.2). This is exactly [0015](./0015-versioning-scheme.md)'s field, unchanged.
...
| Replace 0015 with a fresh versioning ADR                                                       | 0015's package-axis decision is still correct and in force; replacing it would discard a valid decision rather than extend it (§30.2 records 0015 as _kept, extended_).                              |
...
- 0015's package-axis decision survives intact; the chain (0015 → 0041) records _why_ a second axis was added without rewriting history.
...
- Adds: the language version axis (`suspec_language`, `aps_version`, per-file) and the one-way language⇒framework release trigger (§25.1–§25.2); the three distinct version fields in the IR/plan (§25.3).
- Modifies: the scope of ADR-0015 — its single version is now explicitly the _package_ axis, with the language axis tracked separately.
