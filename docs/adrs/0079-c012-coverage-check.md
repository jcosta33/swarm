...
be in scope actually appear in the review — as *the* mechanically defensible wedge for swarm-cli:
...
The `swarm review` M2 spec (SPEC-swarm-cli-m2-review, AC-019/022) needs this reconcile, and the
honesty framework (ADR-0063) forbids swarm-cli claiming a check that has no contract. The capability
...
**Severity is `warning`, deliberately.** A coverage gap is review *completeness*, not artifact
*corruption* (the C001/C003 integrity checks are hard-errors because a malformed spec is objectively
...
**The same check serves two commands at their own exit posture.** `swarm check` may run C012 on
review files at its severity (warning → exit 1); `swarm review` surfaces C012 as one reconcile fact
under its advisory posture (all reconcile findings → exit 1, SPEC-swarm-cli-m2-review AC-024).
...
- `checks.yaml` gains a `C012, name: coverage, severity: warning` core-check row plus a `review_file`
  ...
  counts live).
- **Coordinated cross-repo change (the drift guard).** swarm-cli's `checksContract.ts` pins the
  ...
  `CONTRACT_VERSION` bump, and C012's implementation in swarm-cli **land together** as the M2 build —
  not as a standalone canon commit that would red swarm-cli's gate in between. This ADR records the
  decision; the contract edit ships with the implementation.
- C012 unblocks flipping SPEC-swarm-cli-m2-review from `draft` to `ready`.
