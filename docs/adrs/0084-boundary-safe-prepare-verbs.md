...
The W1 surface map (`suspec-works` AUDIT-suspec-cli-surface) passed the whole candidate suspec-cli surface
through the reconcile-only boundary (ADR-0077) and found exactly one boundary-eroding idea: a
**`status.md`-mutating close** (a `suspec finish`/`suspec close` that flips a board row to a pass-like
...
Two loop verbs remain unbuilt and are boundary-safe (prepare engine, ADR-0077 D1): **`suspec pull`**
...
1. **`suspec pull <ref>` writes exactly one verbatim intake snapshot** (`intake/<slug>.md` with
...
2. **The finding scaffold (`suspec promote`) writes one candidate finding file** (`findings/<slug>.md`
...
3. **The board-mutating close stays PARKED (DECIDE #1.2).** No suspec-cli command writes `status.md`;
   the board is hand-edited. A future `suspec close`/`suspec finish` that mutates the board is a
   ...
   **Reopen trigger:** write that ADR only on demonstrated **adopter demand for the review-gate
   teeth** — generation-volume / convenience demand does not qualify (refuted; suspec-works #1.2, and
...
| ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Ship a board-mutating `suspec close`/`finish` now**                                | Crosses ADR-0077 D8 (the CLI would adjudicate the human-owned result by writing a pass-like board row); it is the open DECIDE #1.2, deliberately parked, not shipped by convenience. Rejected.           |
...
| **`suspec pull` auto-drafts a spec from the ticket**                                 | Normalizing a ticket into requirements is judgment, not transcription; auto-drafting smuggles authoring into a prepare verb. Rejected — `pull` writes the snapshot only.                                 |
...
Accepted. `suspec pull` and `suspec promote` ship as prepare verbs that scaffold one new file each and
...
implementation is cut and reviewed from `suspec-works` (SPEC-suspec-cli-m3-prepare). Negative: two more
