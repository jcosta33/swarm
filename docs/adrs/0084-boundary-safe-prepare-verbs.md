...
The W1 surface map (`swarm-hq` AUDIT-swarm-cli-surface) passed the whole candidate swarm-cli surface
through the reconcile-only boundary (ADR-0077) and found exactly one boundary-eroding idea: a
**`status.md`-mutating close** (a `swarm finish`/`swarm close` that flips a board row to a pass-like
...
pass-like state *is* adjudicating that human-owned verdict. ADR-0077 **Decision 1** enumerates a
"gated close" as a *reconcile* capability; that phrasing is precisely what the open DECIDE #1.2
contests, and it is the clause this ADR narrows.

Two loop verbs remain unbuilt and are boundary-safe (prepare engine, ADR-0077 D1): **`swarm pull`**
(snapshot a ticket into `intake/`) and a **finding scaffold** (offered at Close — scaffold a finding
file from a finished task). They *prepare* a new file; they read or derive no board state.

## Decision

1. **`swarm pull <ref>` writes exactly one verbatim intake snapshot** (`intake/<slug>.md` with
   `source`/`url`/`captured`), and **never a spec** — normalizing a ticket into requirements is
   judgment work, not transcription. Prepare engine (D1); *level: toolable.*
2. **The finding scaffold (`swarm promote`) writes one candidate finding file** (`findings/<slug>.md`
   from the template, pre-filling `from:`), and **never asserts a learning and never writes the
   board**. It is offered at Close; the human fills and accepts it. Prepare engine (D1); *level:
   toolable.* This reverses `future-cli.md`'s "promote folds into close, not its own engine" — the W1
   map re-derived it as its own boundary-safe scaffold verb; recorded here.
3. **The board-mutating close stays PARKED (DECIDE #1.2).** No swarm-cli command writes `status.md`;
   the board is hand-edited. A future `swarm close`/`swarm finish` that mutates the board is a
...
|---|---|
| **Ship a board-mutating `swarm close`/`finish` now** | Crosses ADR-0077 D8 (the CLI would adjudicate the human-owned result by writing a pass-like board row); it is the open DECIDE #1.2, deliberately parked, not shipped by convenience. Rejected. |
...
| **`swarm pull` auto-drafts a spec from the ticket** | Normalizing a ticket into requirements is judgment, not transcription; auto-drafting smuggles authoring into a prepare verb. Rejected — `pull` writes the snapshot only. |
