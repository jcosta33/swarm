...
4. **The `author` pass shipped no guide at all** — nothing taught an agent how to author a `*.swarm.md` spec, an audit, a research doc, or a bug-report *the Swarm way* (SOL blocks, the artifact contracts, the epistemic stances).
...
| Leave the `author` pass guide-less | Leaves agents with no conditioning for authoring the Swarm way — the highest-leverage gap. |
