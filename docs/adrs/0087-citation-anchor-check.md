...
`docs/research/sources.md`. The 2026-06 adversarial audit (swarm-hq #29) found this rule had **no
checker**: `swarm check` validates a spec's frontmatter `sources:` paths (C009), but not its inline
`[[KEY]]` citations. A real dangling citation — `[[FAROS2025]]` in `SPEC-review-gate-benchmark`, an
anchor that does not exist in `sources.md` — passed `swarm check` clean and shipped. The rule's
...
1. **Mint `C015 citation-resolves` (warning, toolable).** A spec's inline `[[KEY]]` citation that does not
   ...
   *Level: toolable (ADR-0063) — `swarm check` flags it. Never enforced (no merge block) until a wired
   gate exists.* It **surfaces a fact** (a dangling citation), never a verdict (ADR-0077 Decision 8):
...
4. **Scope v0 to the dangling-anchor case only.** A `[[KEY]]` with no `<a id="KEY">` is the whole v0.
   **Deferred to a separate v1 decision:** the tier checks (a MUST-level claim citing a *Caveated* or
   *Rejected* entry) — they need the tier metadata parsed out of `sources.md` and a notion of
...
5. **Measured before shipping (the gate).** C015's precision is measured before it counts as shipped:
   run it over the real swarm-hq specs (all citations valid after the #29 remediation) — it must produce
   **zero** C015 warnings (0% effective-FP on real clean specs) — and over a seeded fixture with a
   ...
   the `swarm-bench` corpus is the richer follow-up.
...
6. **Single-sourced like every contract change (the two-repo rule).** `checks.yaml` gains the C015 row +
   ...
   `advanced/checks-reference.md` gains the C015 row. The contract data + the swarm-cli implementation
...
|---|---|
| **CI-lint only** (a grep, no contract mint) | Catches the dangle but lives outside the C0xx contract — adopters' `swarm check` would not surface it, and it would not appear in the kit's catalogue. The owner chose the contract mint for parity with the other citation/structure rules. |
...
| **Global sources.md config** | A `.swarm/config.yaml` `sources_path` would add config surface for a v0; reading the path from the spec's own frontmatter `sources:` is self-contained and matches how the dangling case actually arose. |
...
is *enforced* (a dangling citation never blocks a merge) until a gate wires `swarm check` to CI
(swarm-hq #13/#10). Honors ADR-0077 D8 (a fact, never a verdict), ADR-0063 (toolable, never enforced),
and the single-sourcing rule. Refines ADR-0086 (closes the citation-validation gap it surfaced as #29).

## Propagation

`checks/checks.yaml` (C015 row + version 0.8.0) · `swarm-cli` (`parseSpecRecord` citation marking,
...
(a dangling-citation oracle) · `../swarm-starter-kit/advanced/checks-reference.md` (C015 row) ·
`docs/adrs/README.md` (the index row). The swarm-cli code + the `checks.yaml` data land in one
