...
[ADR-0051](./0051-complete-the-spec-repo-pivot.md) put specs top-level but left the _home_ of every other
...
- `docs/artifacts/memory.md` said findings + ADRs are **"committed source-docs"** (i.e. _not_ under `.agents/`).
...
1. **Feature-scoped artifacts live in a per-feature folder, `specs/<NNN-feature-slug>/`.** The folder holds
   ...
   input (e.g. a research note with no spec yet) simply _starts_ the feature folder it explores. This replaces
...
3. **Durable recall lives in `.agents/memory/`** — `finding`s, `patterns/`, `glossary`, and `INDEX.md` (the
   load-_when_ map). This is unchanged; it is where promotion already targets. The memory `INDEX` may point at
