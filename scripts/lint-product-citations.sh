...
# provenance markers. Those markers are how the corpus repo records WHY a decision was made
# (ADR-NNNN), what audit surfaced a fact (AUDIT-...), and where a primary source lives
# (github / doi.org URLs). They belong in the docs/specs/ADRs of the corpus workspace, NOT in
...
# corpus-mcp/src/tools.ts) grep cannot tell a forbidden citation in prose from the same string
...
#   corpus-skills/skills/<name>/SKILL.md          + that skill's references/*.md
#   corpus-agents/agents/*.md                     (top-level agent definitions)
#   corpus-starter-kit/.agents/skills/<name>/SKILL.md
#   corpus-skills/README.md , corpus-mcp/README.md   (the two product READMEs)
#
...
#     REPO_PARENT_DIR  directory holding the sibling repos (corpus-skills, corpus-agents, …).
...
if [ ! -d "$DEV_DIR" ]; then
    ...
    echo "  Pass the directory that holds the sibling repos (corpus-skills, corpus-agents, …)," >&2
...
files=$(
    # corpus-skills: each skill's SKILL.md + that skill's references/*.md
    if [ -d "$DEV_DIR/corpus-skills/skills" ]; then
        find "$DEV_DIR/corpus-skills/skills" -type f \
            \( -name 'SKILL.md' -o -path '*/references/*.md' \)
    fi
    # corpus-agents: top-level agent definitions only
    if [ -d "$DEV_DIR/corpus-agents/agents" ]; then
        find "$DEV_DIR/corpus-agents/agents" -maxdepth 1 -type f -name '*.md'
    fi
    # corpus-starter-kit: bundled skill guides (.claude/skills is a symlink to this)
    if [ -d "$DEV_DIR/corpus-starter-kit/.agents/skills" ]; then
        find "$DEV_DIR/corpus-starter-kit/.agents/skills" -type f -name 'SKILL.md'
    fi
    ...
    for r in "$DEV_DIR/corpus-skills/README.md" "$DEV_DIR/corpus-mcp/README.md"; do
...
if [ -z "$files" ]; then
    ...
    echo "  Expected sibling repos like $DEV_DIR/corpus-skills, $DEV_DIR/corpus-agents." >&2
...
if [ -n "$hits" ]; then
    ...
    echo "  corpus workspace docs and state only the rule in the product file." >&2
    exit 1
fi
