...
## The load-when discipline (normative, §23.1.1)
...
weight against the loss budget (§24) and the bootloader density cap (§31).

## Durable findings

<!-- One row per promoted/accepted finding.md in the Tier-2 store. Link the file; never restate
     its claim. The Load when mirrors the finding's applies_when scope envelope (§23.3).
...
<!-- One row per patterns/*.md or other Tier-2 topic artifact (§23.2); a pattern distils ≥2
