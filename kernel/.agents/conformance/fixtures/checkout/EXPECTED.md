...
- Adopted-project paths use `.swarm/` throughout (e.g. `.swarm/sources/specs/checkout.swarm.md`).

## How this is validated (no runtime)

This is **inert data** (NO RUNTIME). Swarm ships no parser, linter, lowerer, or checker;
nothing in this directory executes. The verdicts above are **known independent of any tool** and
are **validated by hand** until a checker exists. When a tool does exist it MAY validate against
these files; this manifest is the expected-outcome contract it would be checked against, not a
tool Swarm provides.
