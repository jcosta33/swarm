...
How to execute the `review` pass and decide its merge gate. It is a **pass guide**: SOFT control conditioning how an agent runs the pass; it constrains nothing on its own. It does **not** redefine a verdict's meaning, the proof-strength order, the gate predicate, or waiver/contradiction semantics — those are fixed by the verification rules ([`../../passes/review.md`](../../passes/review.md), with proof strength in [`../../passes/verify.md`](../../passes/verify.md)), which this guide only applies. Every "gate", "check", or "enforcement" below is **manual today**: a contract performed by hand, with a deterministic home (CI, a PreToolUse hook, a merge-blocking status) when a harness exists. Never claim it runs today.
...
- `*.swarm.trace.md` — the implementation's claims (the *input* to judgment; a `TRACE` is never itself a verdict, [`../../passes/review.md`](../../passes/review.md)).
- The `spec.swarm.md` obligations in scope — every `REQ`, `CONSTRAINT`, `INVARIANT`, `INTERFACE` with its required `VERIFY BY` bindings (the merge gate, [`../../passes/review.md`](../../passes/review.md)).
- The recorded verification evidence ([`../../passes/verify.md`](../../passes/verify.md)) and the diffs.
- The `review.md` artifact contract ([`../../templates/review.md`](../../templates/review.md)) for the output shape.
- The default proof suite for the `review` task kind: `manual @ REVIEW` over the recorded evidence, plus re-run of the bound `cmd*` proofs ([`../../passes/verify.md`](../../passes/verify.md)).

The verdict vocabulary, proof-strength order, gate predicate, and waiver/contradiction semantics are fixed by the verification rules ([`../../passes/review.md`](../../passes/review.md) and [`../../passes/verify.md`](../../passes/verify.md)); this guide applies them, never restating their meaning.

## Produces

`review.md` — that artifact **is** the verdict record, the canonical container of `VERDICT` blocks ([`../../passes/review.md`](../../passes/review.md), [`../../templates/review.md`](../../templates/review.md)). The kernel ships **no** `verdict.md`; a repo recording verdicts in a standalone `verdict.md` is non-conformant ([`../../passes/review.md`](../../passes/review.md)). Do not create one.
...
## Rejects — non-proofs that never earn a `PASS` ([`../../passes/verify.md`](../../passes/verify.md))
...
A reviewer who cannot tell whether a binding `BLOCKED` (environment fix) or is `UNVERIFIED` (binding/execution gap) MUST record `UNVERIFIED` — the weaker, more honest claim ([`../../passes/review.md`](../../passes/review.md)). *Why:* `BLOCKED` quietly claims "the truth is merely unknown"; defaulting to it lets an unbound proof masquerade as a transient environment problem.

## Procedure

1. **Read the trace and the obligations side by side.** For every required obligation in scope, find the trace claim that purports to satisfy it and the binding(s) it cites. List the required `VERIFY BY` bindings: **one verdict per binding** ([`../../passes/verify.md`](../../passes/verify.md)), so an obligation with three required bindings owes three verdicts. *Why:* counting verdicts against bindings surfaces an obligation claimed but only partly proven.
...
- ❌ **"Tests passed" as a verdict** → `UNVERIFIED` until a command, exit code, and run output appear in `EVIDENCE`. The bare phrase is the headline non-proof ([`../../passes/verify.md`](../../passes/verify.md)).
- ❌ **Guessing a project command** when `AGENTS.md > Commands` has no slot for it → ask the user; never substitute a command you assume.
- ❌ **Picking the convenient side of two disagreeing proofs** → exactly what `CONTRADICTED` forbids; record both refs and route per the contradiction-resolution rule ([`../../passes/review.md`](../../passes/review.md)). The stronger oracle is a *working assumption*, not a resolution.
- ❌ **Inventing or re-defining a verdict value** (a fifth core, a new decorator, "PASS-ish") → the seven values and their decoration rules are fixed by the verdict model ([`../../passes/review.md`](../../passes/review.md)); this guide only applies them.
...
- ❌ **Creating a `verdict.md`** → `review.md` *is* the verdict record ([`../../passes/review.md`](../../passes/review.md)); a standalone `verdict.md` is non-conformant.
- ❌ **Claiming the gate "runs" or "is enforced" today** → it is manual today ([`../../passes/review.md`](../../passes/review.md)); name its future deterministic home, do not assert automatic enforcement.
...
- Is there **one verdict per required binding** ([`../../passes/verify.md`](../../passes/verify.md)), and does `## Final verdict` follow strictly from the gate predicate ([`../../passes/review.md`](../../passes/review.md)) — no `FAIL`/`BLOCKED`/`UNVERIFIED`/`STALE`/`CONTRADICTED` left in a required binding while you wrote `PASS`?
- For every `manual` verdict: is the judge identity recorded, is the judge independent of the implementer, and is the model-judge discipline ([`../../passes/verify.md`](../../passes/verify.md)) (lineage, separation of duties, dual judgment for high risk) satisfied?
